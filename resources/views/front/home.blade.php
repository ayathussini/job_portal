    ...
    <div class="container">
        ...
        <div class="row pt-5">
            <div class="job_listing_area">                    
                <div class="job_lists">
                    <div class="row">
                        @if( $latestdJobs->isNotEmpty())
                             @foreach (  $latestdJobs as  $latestdJob )
                               <div class="col-md-4">
                            <div class="card border-0 p-3 shadow mb-4">
                                <div class="card-body">
                                    <h3 class="border-0 fs-5 pb-2 mb-0">{{ $latestdJob->title}}</h3>
                                    <p>{{Str::words( strip_tags($latestdJob->description),5)}}</p>
                                    <div class="bg-light p-3 border">
                                        <p class="mb-0">
                                            <span class="fw-bolder"><i class="fa fa-map-marker"></i></span>
                                            <span class="ps-1">{{ $latestdJob->location}}</span>
                                        </p>
                                        <p class="mb-0">
                                            <span class="fw-bolder"><i class="fa fa-clock-o"></i></span>
                                            <span class="ps-1">{{ $latestdJob->jobType->name}}</span>
                                        </p>
                                        @if (is_null( $latestdJob->salary))
                                        <p class="mb-0">
                                            <span class="fw-bolder"><i class="fa fa-usd"></i></span>
                                            <span class="ps-1">{{ $latestdJob->salary}}</span>
                                        </p>
                                    ...
                                    <div class="d-grid mt-3">
                                        <a href="{{ route('jobDetail', $latestdJob->id) }}" class="btn btn-primary btn-lg">Details</a>
                                    </div>
                                </div>
                            </div>
                        </div>
